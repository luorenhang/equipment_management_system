...
| Cxr_Responsible | 设备负责人 | 记录设备的责任人信息 |
...
1. **设备台账与负责人关系**：一个设备可以有多个负责人，通过`Cxr_Responsible.EquipmentLedger`关联`Cxr_EquipmentLedger.ID`
...
4. 实现设备管理的移动端应用
5. 引入分布式事务：Seata
6. 引入服务链路追踪：Sleuth + Zipkin
7. 实现自动化部署：Docker + Kubernetes
8. 引入API文档管理：Swagger/OpenAPI

## 12. 注意事项

1. **Spring Cloud Gateway与Spring MVC冲突**：网关服务使用WebFlux架构，与Spring MVC不兼容，需确保网关服务不依赖包含Spring MVC的模块（如common模块）
2. **服务间通信**：微服务之间使用Feign或RestTemplate进行通信
3. **配置管理**：所有微服务的配置统一管理在Nacos配置中心
4. **服务命名规范**：微服务名称使用小写字母，单词之间用连字符分隔
5. **端口管理**：各微服务使用不同的端口，避免端口冲突
6. **数据库设计**：各微服务可共享同一数据库，也可使用独立数据库，根据业务需求和数据一致性要求决定
